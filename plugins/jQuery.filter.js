$.fn.sx = function(can) {
	can = $.extend({
		nuv: ".filter-result",			//筛选结果
		zi: "select-btn",						//所有筛选范围内的子类
		qingchu: '.clear-result',		//清除全部
		over: 'active'							//选中状态样式名称
	},
	can || {});
/*删选过程*/
	$(this).find('a').click(function() {
		...
		if($(this).attr('class').replace(can.zi,'').replace(/ /g,'')=="active"){
...
/*选中*/
	$("."+can.zi).each(function() {
		...
		if (url.split(pddq).length > 1) {
			...
			$(can.nuv).find(can.qingchu).before("<a rel=" + $(this).attr('rel') + " name=" + $(this).attr('name') + " class='active' href='javascript:void(0);'>" + $(this).siblings('dd').text() + $(this).text() + " <i class='fa fa-close'></i></a> ")
		} else {
